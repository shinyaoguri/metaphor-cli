...
AI エージェント向けの MCP サーバ（JSON-RPC / stdio）です。通常は AI クライアントが自動起動するため、手動で実行する必要はありません。詳細は [AI と協調する](#ai-と協調する) を参照してください。

```bash
metaphor mcp [sketch-dir]   # 省略時はカレントディレクトリのスケッチ
```
...
2. MCP サーバを登録する。`metaphor mcp` は **ユーザーがターミナルで起動するものではなく**、AI クライアントが標準入出力（stdio）経由で **裏側で自動起動する** サーバです。一度登録すれば、あとは AI に話しかけるだけです。

   Claude Code では、スケッチのディレクトリで次を一度実行します。
