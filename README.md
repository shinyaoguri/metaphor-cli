...
metaphor run --syphon=MyOutput   # publish する Syphon サーバ名を明示
...
- `--metrics` — 実測 fps・メモリ・CPU・thermal をターミナル最下行へライブ表示します（下記参照）。
...
metaphor watch --syphon-name MySketch  # publish する Syphon サーバ名を固定
...
既定（ライブビューア）では常設のライブビューア窓を開き、再ビルド時はスケッチ（子プロセス）だけを差し替えます。ウィンドウは閉じず、マウス/キー入力はライブビューアからスケッチへ転送されます。既定で AI からアタッチ観測できる共有セッションも公開します（[AI と協調する](#ai-と協調する)）。
...
`watch` のログには時刻が付きます（長時間つけっぱなしで使うため、どのリロード・ビルド失敗がいつのものかスクロールバックから追えます）。

```
[20:27:04] [watch] Ctrl-C で停止
[20:27:34] [watch] 変更を検出 — 再ビルド中…
[20:27:35] [watch] リロードしました
```

時刻はターミナル（TTY）では控えめな表示になり、ファイルへリダイレクトしたときは装飾なしのプレーンテキストになります。
...
AI エージェント向けの MCP サーバ（JSON-RPC / stdio）です。AI と協調させる＝この MCP を使う、ということなので、**最初に 1 回だけ次を実行して登録します**。
...
2. **最初に 1 回だけ、MCP サーバを登録します。** Claude Code では、スケッチのディレクトリで次を実行します。
...
3. エージェントに依頼します。同じディレクトリで AI クライアントを開き、目的を伝えます。エージェントは `snapshot` で結果を確認しながらコードを修正します。
...
>
> 逆順（Claude Code を先に開き `metaphor mcp` が起動済み）だと、mcp は `watch` を見つけられず**自前の別インスタンス**（ヘッドレス）を観測し、あなたのライブビューア窓とは別物になります。後から再チェックはしません。直すには Claude Code 側で MCP を再接続（`/mcp`）するか開き直すと、`metaphor mcp` が再起動して `watch` にアタッチし直します。
...
- **`.app` に包んだスケッチで snapshot / params が効かない** — `.app` を LaunchServices（`open` / Dock / ログイン項目）から起動すると **cwd が `/`** になり、スケッチが `.metaphor/` を書けない場所に作ろうとします。`METAPHOR_STATE_DIR` に置き場を渡してください（`metaphor run` / `metaphor watch` / `metaphor mcp` 経由なら自動で渡ります）。
  ...
  `metaphor mcp [sketch-dir]` では、`.metaphor/` の基準は **`METAPHOR_STATE_DIR` が優先**（未指定なら `sketch-dir`）。相対パスで渡した場合は CLI 側の cwd を基準に絶対化してから子スケッチへ渡すので、`sketch-dir` が cwd と違っていても AI とスケッチが同じ場所を見ます。
